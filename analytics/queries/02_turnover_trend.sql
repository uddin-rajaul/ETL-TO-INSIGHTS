...
monthly_headcount AS (
    ...
    SELECT
        EXTRACT(YEAR  FROM d.snapshot_date)::int AS year,
        EXTRACT(MONTH FROM d.snapshot_date)::int AS month,
        COUNT(e.id)                              AS active_count
    FROM generate_series(
        (SELECT MIN(hire_date) FROM silver.employee),
        CURRENT_DATE,
        INTERVAL '1 day'
    )::date AS d(snapshot_date)
    LEFT JOIN silver.employee e
        ON e.hire_date <= d.snapshot_date
        AND (e.term_date IS NULL OR e.term_date > d.snapshot_date)
    GROUP BY
        EXTRACT(YEAR  FROM d.snapshot_date),
        EXTRACT(MONTH FROM d.snapshot_date)
)
